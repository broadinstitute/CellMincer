'''Command-line tool functionality for `cellmincer insight`.'''
...
from cellmincer.insight.main import Insight
...
class CLI(AbstractCLI):
    ...
    def run(self, args):
        ...
        log_file = os.path.join(params['log_dir'], 'cellmincer_insight.log')
        logging.basicConfig(
            level=logging.INFO,
            format='cellmincer:insight:%(asctime)s: %(message)s',
            filename=log_file,
            filemode='w')
        console = logging.StreamHandler()
        formatter = logging.Formatter('cellmincer:insight:%(asctime)s: %(message)s', '%H:%M:%S')
        ...
        logging.info('Command:\n' + ' '.join(['cellmincer', 'insight'] + sys.argv[2:]))
